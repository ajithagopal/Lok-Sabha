...
var quizAll = {
    ...
    "lok18": [
        {
            q: 'In the 2024 Lok Sabha polls, which State/UT recorded the highest voter turnout of 84%?',
            o: [
                {'txt': 'Lakshadweep', 'ans': true},
                {'txt': 'Bihar'},
                {'txt': 'Meghalaya'}
            ],
            info: 'Lakshadweep recorded the highest voter turnout at 84% followed by Assam at 81%.'
        },
        {
            q: ' Awadesh Prasad, a Dalit candidate from the Samajwadi Party, made news by registering a victory in which constituency?',
            o: [
                {'txt': 'Faizabad', 'ans' : true},
                {'txt': 'Varanasi'},
                {'txt': 'Muzzafarnagar'}
            ],
            info: 'Two holy sites for Ram rejected the BJP: Samajwadi Party’s Dalit candidate Awadesh Prasad winning from Faizabad (home to Ayodhya).'
        },
        {
            q: 'In which State did the BJP win 19 out of the 21 Lok Sabha seats, thereby unseating the government there?',
            o: [
                {'txt': 'Andhra Pradesh'},
                {'txt': 'Odisha', 'ans': true},
                {'txt': 'Arunachal Pradesh'}
            ],
            info: 'The party registered gains in Odisha, winning 19 of the 21 Lok Sabha seats, drew a blank in Tamil Nadu.'
        }
    ],
}
...
$(document).ready(function(){
    ...
    $("#dynamic-content").on("click", ".movetonext h2 a", function(e) {
        e.preventDefault()
        // if(!$(this).parent("h2").hasClass("active") ){
            let file = $(this).attr("data-file");  
            homePageNav(file)
    ...
    $(".link-list li a").click(function(e) {
        ...
        // if(!$(this).parent("li").hasClass("active") ){
            ...
            //     // playInitialAudio();
            //     window.scrollTo({
            //         top: document.getElementById("dynamic-content").offsetTop - 100,
            //         left: 100,
            //         behavior: "smooth",
            //     });
            // });
    ...
    var link_scroll_length, scroll_length, total_fold, remaining_length, no_of_fold, totalWidth = -3.8, activeLinkWidth = -3.8;
    ...
    window.initScroll = function() {
        ...
        if($( window ).width() > 1200){
            ...
            link_scroll_length = totalWidth - 1150;
            scroll_length = 0;
            total_fold = parseInt((totalWidth - 1150) / 1150);
            remaining_length = (totalWidth - 1150) % 1150;
            ...
            $(".link-list").scrollLeft($(".link-list li.active").offset().left - 50);
        }
    }
...
//     var elementFarLeft = elementLeft + $(this).outerWidth();

//     var viewportTop = $(window).scrollTop();
//     var elementFarLeft = viewportTop + $(window).height();

//     return elementBottom > viewportTop && elementLeft < elementFarLeft;
// };

function homePageNav(file) {
    $.get(`./assets/files/${file}.html`, function( data ) {
        $( "#dynamic-content" ).html( data );
        transitionElements()
        initLightbox()
    ...
    $(".link-list li").removeClass("active")
    let fileElement = $("a[data-file="+file+"]")
    fileElement.parent("li").addClass("active")

    window.initScroll()


    // if(fileElement.offset().left > $(".link-list-outer .link-list").outerWidth()) {
    //     $(".scroll-arrow-btn.right").trigger("click")
    //     setTimeout(() => {
    //         if(fileElement.offset().left > $(".link-list-outer .link-list").outerWidth()) {
    //             $(".scroll-arrow-btn.right").trigger("click")
    //         }
    ...
    // if(fileElement.offset().left < $(".link-list-outer .link-list").outerWidth()) {
    //     $(".scroll-arrow-btn.left").trigger("click")
    //     setTimeout(() => {
    //         if(fileElement.offset().left < $(".link-list-outer .link-list").outerWidth()) {
    //             $(".scroll-arrow-btn.left").trigger("click")
    //         }
    ...
    // console.log(fileElement.offset().left, ' - ', $(".link-list-outer").outerWidth() )
}
